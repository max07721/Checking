				...
				<el-col>
					<el-form :model="form" ref="form" label-width="80px" class="form">
						...
						<el-form-item label="身份" prop="user_group">
							...
							<!-- 学生用户 -->
						<form_student_users v-if="form.user_group === '学生用户'" :form="form_sub" ></form_student_users>
								<!-- 教师用户 -->
						<form_teacher_users v-if="form.user_group === '教师用户'" :form="form_sub" ></form_teacher_users>
...
<script>
	import mixin from "@/mixins/page.js";
		/* 学生用户 */
	import form_student_users from "@/components/diy/form_student_users";
			/* 教师用户 */
	import form_teacher_users from "@/components/diy/form_teacher_users";
			...
			async submit_check(param){
				...
				if(!ret && user_group){
						...
						var form_sub = Object.assign({} ,this.form_sub);
											if(!ret && !form_sub["student_id"]){
							ret = "学生学号不能为空!";
						}
						else if(!ret){
							res = await this.$get("~/api/student_users/count?", {"student_id": form_sub["student_id"]});
							if(res.result){
								ret = "学生学号已存在!";
							}
						}
							...
							if(user_group === "教师用户"){
						var form_sub = Object.assign({} ,this.form_sub);
																if(!ret && !form_sub["teacher_id"]){
							ret = "教师工号不能为空!";
						}
						else if(!ret){
							res = await this.$get("~/api/teacher_users/count?", {"teacher_id": form_sub["teacher_id"]});
							if(res.result){
								ret = "教师工号已存在!";
							}
						}
