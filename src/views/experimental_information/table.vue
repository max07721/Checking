<template>
	<el-main class="bg table_wrap">
		<el-form label-position="right" :model="query" class="form p_4" label-width="120">
			<el-row>
					...
					<el-form-item label="实验题目">
									<el-select v-model="query.experimental_topic">
											<el-option v-for="o in list_experimental_topic" :key="o.experimental_topic" :label="o.experimental_topic"
								:value="o.experimental_topic">
							</el-option>
										</el-select>
								</el-form-item>
				</el-col>
					...
					<el-form-item label="实验地点">
									<el-input v-model="query.experimental_location"></el-input>
								</el-form-item>
				</el-col>
					...
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/experimental_information/table','add') || $check_action('/experimental_information/view','add')" class="el-button el-button--default el-button--primary search_btn_add" to="./view?">添加
						</router-link>
            			<el-button v-if="user_group == '管理员' || $check_action('/experimental_information/table','del') || $check_action('/experimental_information/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/experimental_information/table','set') || $check_action('/experimental_information/view','set') || $check_action('/experimental_information/view','get') || $check_action('/解答报告/table','add') || $check_action('/解答报告/view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/experimental_information/table','set') || $check_action('/experimental_information/view','set') || $check_action('/experimental_information/view','get')"
						:to="'./view?' + field + '=' + scope.row[field]"
						 size="small">
						<span>详情</span>
					</router-link>
						<!--跨表按钮-->
							<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="to_table(scope.row,'/answer_record/view')" v-if="user_group == '管理员' || $check_action('/answer_record/table','add') || $check_action('/answer_record/view','add')">
						<span>解答报告</span>
					</el-button>
							</template>
			</el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/experimental_information/get_list?like=0",
				url_del: "~/api/experimental_information/del?",

				// 字段ID
				field: "experimental_information_id",
		...
		methods: {
			 ...
			 * 获取实验题目列表
			 */
			async get_list_experimental_topic() {
				var json = await this.$get("~/api/experimental_topic/get_list?");
				if(json.result){
					this.list_experimental_topic = json.result.list;
				}else if (json.error){
					console.log(json.error);
				}
			},
		...
		created() {
			...
			this.get_list_course_name();
						// 初始化实验题目列表
			this.get_list_experimental_topic();
								}
	}
</script>
