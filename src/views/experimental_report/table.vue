<template>
    ...
    <el-form label-position="right" :rules="rules" ref="formDate" :model="formDate" class="form formDare p_4" label-width="120">
      <el-row>
        <el-col :xs="24" :sm="24" :lg="8" class="el_form_search_wrap">
          <el-form-item prop="targetId">
            <el-input v-model="formDate.targetId" clearable></el-input>
          </el-form-item>
          <el-form-item prop="resourcesId">
            <el-input v-model="formDate.resourcesId" clearable></el-input>
          </el-form-item>
        </el-col>
        <el-col :xs="24" :sm="10" :lg="8" class="search_btn_wrap_1">
          <el-form-item>
            <el-button type="primary"  @click="searchFn"  class="search_btn_find">查重</el-button>
            <el-button  @click="resetFn"  class="search_btn_reset reset">重置</el-button>
          </el-form-item>
        </el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%"
          ...
          <el-button class="el-button el-button--small is-plain el-button--default"
                     style="margin: 5px !important;" size="small" @click="exportPDF(scope.row[field])"
                     v-if="user_group == '管理员' || $check_action('/experimental_report/table','set') || $check_action('/experimental_report/view','set') || $check_action('/experimental_report/view','get')">
            <span>PDF导出</span>
          </el-button>
    ...
    <el-dialog title="提示" :visible.sync="dialogVisible" width="30%" style="border-radius: 8px" >
      <span>{{text}}</span>
...
<script>
	...
	export default {
      ...
      btnFn(data){
        console.log(data)
        if (this.formDate.targetId&&this.formDate.resourcesId) return this.$message.warning('已经满了，请先删除一个')
        if(this.formDate.targetId){
          this.formDate.resourcesId = `${data.student_name}-${data.course_name}-${data.experimental_topic}`
          this.id1 = data.experimental_report_id
        }else{
          this.formDate.targetId = `${data.student_name}-${data.course_name}-${data.experimental_topic}`
          this.id2 = data.experimental_report_id
        }
      },
      searchFn(){
        this.$refs.formDate.validate(isOk=>{
          if(isOk){
            this.$get(`~/api/experimental_report/similarity?targetId=${this.id1}&resourcesId=${this.id2}`).then(res=>{
              this.dialogVisible = true
              this.text = `查重率：${res.result}`
            })
            }
        })
      },
