	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
				...
				<el-form-item label="教师账号" prop="teacher_account">
													<el-select v-if="user_group === '管理员' || (form['answer_record_id'] && $check_field('set','teacher_account')) || (!form['answer_record_id'] && $check_field('add','teacher_account'))" id="teacher_account" v-model="form['teacher_account']" :disabled="disabledObj['teacher_account_isDisabled']">
				...
				<el-form-item label="教师姓名" prop="teachers_name">
							  ...
							  v-if="user_group === '管理员' || (form['answer_record_id'] && $check_field('set','teachers_name')) || (!form['answer_record_id'] && $check_field('add','teachers_name'))" :disabled="disabledObj['teachers_name_isDisabled']"></el-input>
				...
				<el-form-item label="学生账号" prop="student_account">
													<el-select v-if="user_group === '管理员' || (form['answer_record_id'] && $check_field('set','student_account')) || (!form['answer_record_id'] && $check_field('add','student_account'))" id="student_account" v-model="form['student_account']" :disabled="disabledObj['student_account_isDisabled']">
				...
				<el-form-item label="学生姓名" prop="student_name">
							  ...
							  v-if="user_group === '管理员' || (form['answer_record_id'] && $check_field('set','student_name')) || (!form['answer_record_id'] && $check_field('add','student_name'))" :disabled="disabledObj['student_name_isDisabled']"></el-input>
				...
				<el-form-item label="课程名称" prop="course_name">
							  ...
							  v-if="user_group === '管理员' || (form['answer_record_id'] && $check_field('set','course_name')) || (!form['answer_record_id'] && $check_field('add','course_name'))" :disabled="disabledObj['course_name_isDisabled']"></el-input>
				...
				<el-form-item label="实验题目" prop="experimental_topic">
							  ...
							  v-if="user_group === '管理员' || (form['answer_record_id'] && $check_field('set','experimental_topic')) || (!form['answer_record_id'] && $check_field('add','experimental_topic'))" :disabled="disabledObj['experimental_topic_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','experimental_topic')">{{form['experimental_topic']}}</div>
											</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','experimental_location') || $check_field('add','experimental_location') || $check_field('set','experimental_location')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="实验地点" prop="experimental_location">
							  ...
							  v-if="user_group === '管理员' || (form['answer_record_id'] && $check_field('set','experimental_location')) || (!form['answer_record_id'] && $check_field('add','experimental_location'))" :disabled="disabledObj['experimental_location_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','experimental_location')">{{form['experimental_location']}}</div>
											</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','experimental_content') || $check_field('add','experimental_content') || $check_field('set','experimental_content')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="实验内容" prop="experimental_content">
								<el-input type="textarea" id="experimental_content" v-model="form['experimental_content']" placeholder="请输入实验内容"
						v-if="user_group === '管理员' || (form['answer_record_id'] && $check_field('set','experimental_content')) || (!form['answer_record_id'] && $check_field('add','experimental_content'))" :disabled="disabledObj['experimental_content_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','experimental_content')">{{form['experimental_content']}}</div>
							</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','experimental_time') || $check_field('add','experimental_time') || $check_field('set','experimental_time')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="实验时间" prop="experimental_time">
								<el-date-picker :disabled="disabledObj['experimental_time_isDisabled']" v-if="user_group === '管理员' || (form['answer_record_id'] && $check_field('set','experimental_time')) || (!form['answer_record_id'] && $check_field('add','experimental_time'))" id="experimental_time"
						v-model="form['experimental_time']" type="datetime" placeholder="选择日期时间">
					</el-date-picker>
					<div v-else-if="$check_field('get','experimental_time')">{{form['experimental_time']}}</div>
							</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','experimental_solution') || $check_field('add','experimental_solution') || $check_field('set','experimental_solution')" :xs="24" :sm="24" :lg="24" class="el_form_editor_warp">
				<el-form-item label="实验解答" prop="experimental_solution">
					<quill-editor v-model.number="form['experimental_solution']"
						v-if="user_group === '管理员' || (form['answer_record_id'] && $check_field('set','experimental_solution')) || (!form['answer_record_id'] && $check_field('add','experimental_solution')) ">
					</quill-editor>
					<div v-else-if="$check_field('get','experimental_solution')" v-html="form['experimental_solution']"></div>
				</el-form-item>
			</el-col>
	...
	export default {
		...
		data() {
			return {
				...
				url_add: "~/api/answer_record/add?",
				url_set: "~/api/answer_record/set?",
				url_get_obj: "~/api/answer_record/get_obj?",
				url_upload: "~/api/answer_record/upload?",
		...
		methods: {
            ...
            this.form["experimental_time"] = this.$toTime(parseInt(this.form["experimental_time"]),"yyyy-MM-dd hh:mm:ss")
        }
								$.db.del("form");
				return param;
			},
          ...
          this.form["experimental_time"] = null;
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/answer_record/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/answer_record/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/answer_record/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/answer_record/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/answer_record/view','get');
